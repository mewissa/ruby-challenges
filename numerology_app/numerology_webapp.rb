...
def valid_birthdate(bdate)
	if bdate.length == 8 && bdate.match(/^[0-9]+[0-9]$/)
		return true
	else
		return false
	end
...
post '/' do
	...
	if valid_birthdate(params[:birthdate]) == true
		setup_index_view
		birth_path_num = @birth_path_num
		redirect "/message/#{birth_path_num}"
	else
		@error = "Make sure your birthdate has been entered in MMDDYYYY form!"
		erb :form
	end
